...
class VizDoomReward:
    ...
    def __call__(
        ...
        rwd_health = 0.1 * (game_var["HEALTH"] - game_var_old["HEALTH"])
        rwd_dead = -10.0 if (game_var["DEAD"] - game_var_old["DEAD"]) > 0 else 0.0
        rwd_ammo = -0.5 * (game_var_old["AMMO"] - game_var["AMMO"]) if "AMMO" in game_var else 0.0

        return (rwd_hit, rwd_hit_taken, rwd_frag, rwd_health, rwd_dead, rwd_ammo)